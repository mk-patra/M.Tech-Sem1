#include <stdio.h>
int num;
void stam(int a[][num], int b[][num], int c[][num], int size) 
{
	int p1[size/2][size/2], p2[size/2][size/2], p3[size/2][size/2];
	int p4[size/2][size/2], p5[size/2][size/2], p6[size/2][size/2], p7[size/2][size/2];
...
int main() 
{
	...
	for(i = temp; i < num; i++)
		for(j = 0; j < num; j++)
			b[i][j] = 0;
	printf("Matrix a:\n"); //printing the actual matrices for stam's multiplication
	for(i = 0; i < num; i++) {
		for(j = 0; j < num; j++) {
			printf("%d ", a[i][j]);
		}
